...
async function readFromGitHub() {
  ...
  if (!data?.reliefPlans || !data.updatedAt) return null;
  const hasPlans = Object.keys(data.reliefPlans).length > 0;
  const hasScores = Object.keys(data.reliefScore || {}).length > 0;
  if (!hasPlans && !hasScores) return null;
  return data;
}
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    return {
      statusCode: 500,
      headers: corsHeaders,
      body: JSON.stringify({ error: err.message || "Gagal baca relief cloud" })
    };
  }
};